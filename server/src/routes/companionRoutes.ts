...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const companion = await companionService.getCompanion(req.user!.id);
    res.json(companion);
  } catch (error) {
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const companion = await companionService.createCompanion(req.user!.id, name, type);
    res.json(companion);
  } catch (error) {
...
router.post('/interact', authenticateToken, async (req, res) => {
    try {
        ...
        const companion = await companionService.addXp(req.user!.id, xpGain);
        res.json(companion);
    } catch (error) {
