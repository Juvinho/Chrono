    ...
    const aggregatedNotifications = React.useMemo(() => {
        ...
        notifications.forEach(n => {
            ...
            if (n.post?.id) {
                ...
                // For follow, group all recent follows together if they are unread? 
                // Or just show them individually? Usually social networks group "X and 5 others followed you".
                key += '-global-follow'; 
            } else {
    ...
    return (
        <div className="notifications-panel">
            ...
            {aggregatedNotifications.length > 0 ? (
                <div>
                    {aggregatedNotifications.map(n => <NotificationItem key={n.key} notification={n} onNotificationClick={onNotificationClick} />)}
                </div>
            ) : (
