...
import { generatePostContent, generateReplyContent, generatePollVote, generateDirectMessageReply } from './utils/geminiService';
import { apiClient, mapApiUserToUser, mapApiPostToPost, mapApiStoryToStory } from './api';
...
export default function App() {
    ...
    const handleRequestConnection = async (username: string) => {
        if (!currentUser) return;
        try {
            const result = await apiClient.requestConnection(username);
            if (result.error) {
                showToast(result.error, 'error');
                return;
            }
            showToast(t('connectionRequestSent') || 'Pedido de conexão enviado!', 'success');
            await reloadBackendData();
        } catch (error) {
            console.error("Failed to request connection:", error);
            showToast('Erro ao solicitar conexão', 'error');
        }
    };
    ...
    const handleAcceptConnection = async (requestId: string) => {
        if (!currentUser) return;
        try {
            const result = await apiClient.acceptConnection(requestId);
            if (result.error) {
                showToast(result.error, 'error');
                return;
            }
            showToast(t('connectionAccepted') || 'Conexão aceita!', 'success');
            await reloadBackendData();
        } catch (error) {
    ...
    const handleDeclineConnection = async (requestId: string) => {
        if (!currentUser) return;
        try {
            const result = await apiClient.declineConnection(requestId);
            if (result.error) {
                showToast(result.error, 'error');
                return;
            }
            showToast(t('connectionDeclined') || 'Conexão recusada', 'info');
            await reloadBackendData();
        } catch (error) {
